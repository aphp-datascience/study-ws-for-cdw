...
def train(
    task_ids: List[int],
    batch_size: int = 32,  # Total batch size across all processes
    model_name: Optional[str] = None,
    path_to_training_datasets: str = f"{PATH_DATA_SCRATCH}/wedsak_datasets/train/",
    ...
    training_datasets_hash = {
        task_name: hash_file_or_directory(Path(path_to_training_datasets, task_name))
    ...
    _ = nlp.add_pipe(
        eds.span_classifier(
            ...
            span_getter=["dates"] + task_names,
            attributes={"_." + t: ["date", "train_" + t] for t in task_names},
    ...
    for task_name in task_names:
        training_data = get_dataset(
            path=path_to_training_datasets,
            task_name=task_name,
            span_label="train_" + task_name,
    ...
    # Making the stream of mini-batches for each task
    device = "cuda" if torch.cuda.is_available() else "cpu"
    batch_loaders = []

    # Nombre de steps réel : max_steps x n_accumulation_steps
    n_accumulation_steps = len(training_datasets)
    logger.info("Number of accumulation steps: %d", n_accumulation_steps)

    logger.info("Preparing data loaders")
    for task_name, dataset in zip(task_names, training_datasets):
        batches = dataset.loop().map(eds.explode(span_getter=task_names))
    ...
    logger.info("Creating iterators for each task")
    iterators = {
        task_name: iter(batch_loader)
        for task_name, batch_loader in zip(task_names, batch_loaders)
    }
    logger.info("Iterators: %s", list(iterators.keys()))

    # Move the model to the GPU/accelerator device
    nlp.to(device)
    ...
    loss_tracking = {task: [] for task in task_names}
    metrics_tracking = {task: [] for task in task_names}

    # Get reference docs and gold labels for evaluation (main process only to avoid duplication)
        ...
        for task_name, iterator in iterators.items():
            ...
            with nlp.cache():
                for name, component in nlp.torch_components():
                    ...
                    if "loss" in output:
                        loss = output["loss"] / n_accumulation_steps
                        loss.backward()

            loss_tracking[task_name].append(loss.item())
            if ((step % log_every_n_steps) == 0) or (step == max_steps - 1):
                loss_rolling_mean = np.mean(
                    loss_tracking[task_name][-log_every_n_steps:]
                ).item()
                ...
                if ref_docs is not None and gold is not None:
                    span_classifier = nlp.get_pipe("span_classifier")
                    ...
                    save_json(
                        metadata, sub_folder="train", file_name=file_name_metadata
                    )
                    nlp.train(True)
